...
@dg.asset(
    ...
    ins={
        "dataset": AssetIn(key="llm_enriched__dataset"),
    },
)
def deanery_filled__dataset(
...
@dg.asset(
    ...
    kinds={Kinds.PYTHON, Kinds.PYDANTIC},
    ins={
        "gt_parsed_dataset": AssetIn(key="gt__parsed_dataset__pydantic"),
        "gt_source_dataset": AssetIn(key="gt__source_dataset__pydantic"),
        "parsed_dataset": AssetIn(key="parsed__dataset"),
    },
)
def gt_aligned__dataset(
    context: AssetExecutionContext,
    gt_parsed_dataset: BaseDataset[GroundTruthDataItem],
    gt_source_dataset: BaseDataset[GroundTruthDataItem],
    parsed_dataset: BaseDataset[PredictionDataItem],
    ...
    Args:
        ...
        parsed_dataset: Parsed predictions dataset with SchematismPage entries
    ...
    datasets = {"source": gt_source_dataset, "parsed": gt_parsed_dataset}

    gt_dataset = datasets.get(config.ground_truth_source, None)
    ...
    parsed_by_id = {
        item.metadata.sample_id: item for item in parsed_dataset.items if item.metadata
    }
    ...
    for sample_id in sorted(common_sample_ids):
        ...
        aligned_gt_entries, aligned_pred_entries = aligner.align_entries(
            {"entries": [entry.model_dump() for entry in gt_item.ground_truth.entries]},
            {
                "entries": [
    ...
    context.add_output_metadata(
        {
            ...
            "parsed_dataset_size": MetadataValue.int(len(parsed_dataset.items)),
...
@dg.asset(
    ...
    kinds={Kinds.PYTHON, Kinds.PYDANTIC},
    ins={
        "dataset": AssetIn(key="deanery_filled__dataset"),
