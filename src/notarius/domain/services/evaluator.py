...
from notarius.schemas.data.metrics import Metrics, PageDataMetrics
...
def align_page_data(
    ...
    for i, gt_parish in gt_parishes.items():
        ...
        unmatched_preds = {
            p_val: p_idx
            for p_idx, p_val in pred_parishes.items()
            if p_idx not in matched_pred_indices and p_val
        }
...
def evaluate_json_response(
    predictions_data: SchematismPage,
    ground_truth_data: SchematismPage,
    fuzzy_threshold: int = 80,
    scorer: Callable[..., int] = fallback_scorer,
) -> PageDataMetrics:
    entry_fields = list(SchematismEntry.model_fields.keys())
    metrics = {field: Metrics() for field in ["page_number", *entry_fields]}
