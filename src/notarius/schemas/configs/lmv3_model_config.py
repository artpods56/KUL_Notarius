...
class ModelConfig(BaseModel):
    checkpoint: str = Field(
        ...
        description="Model checkpoint path or identifier",
    )


class RunConfig(BaseModel):
    device: str = Field(
        default="cpu", description="Device to run the model on (cpu, cuda, etc.)"
    )
...
class ProcessorConfig(BaseModel):
    checkpoint: str = Field(
        default="microsoft/layoutlmv3-base",
        description="Processor checkpoint path or identifier",
    )
...
class TrainingConfig(BaseModel):
    output_dir: str = Field(
        default="./output", description="Output directory for model checkpoints"
    )
    max_steps: int = Field(default=100, description="Maximum number of training steps")
    per_device_train_batch_size: int = Field(
        default=1, description="Batch size per device for training"
    )
    per_device_eval_batch_size: int = Field(
        default=1, description="Batch size per device for pipeline"
    )
    ...
    save_total_limit: int = Field(
        default=4, description="Maximum number of checkpoints to keep"
    )
...
class InferenceConfig(BaseModel):
    checkpoint: str = Field(
        default="microsoft/layoutlmv3-base",
        description="Checkpoint path or identifier for inference",
    )
...
class MetricsConfig(BaseModel):
    return_entity_level_metrics: bool = Field(
        default=True, description="Return entity level metrics"
    )
...
class BaseLMv3ModelConfig(BaseModel):
    ...
    processor: ProcessorConfig = Field(
        default_factory=ProcessorConfig, description="Processor configuration"
    )
    focal_loss: FocalLossConfig = Field(
        default_factory=FocalLossConfig, description="Focal loss configuration"
    )
