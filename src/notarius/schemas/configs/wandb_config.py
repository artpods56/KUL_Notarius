...
from notarius.infrastructure.config.constants import ConfigType, WandbConfigSubtype
...
class WandbConfig(BaseModel):
    ...
    project: str = Field(
        default="ai-osrodek", description="Weights & Biases project description"
    )
    entity: str = Field(default="", description="Weights & Biases entity description")
    name: str = Field(
        default="experiment", description="Weights & Biases run description"
    )
    ...
    num_prediction_samples: int = Field(
        default=15, description="Number of prediction samples to log"
    )
